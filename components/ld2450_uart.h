  ...
  void handlePeriodicData(char *buffer, int len) {
    ...
    */
    ...
    if ((target1Resolution->get_state() == 0) &&
          (target2Resolution->get_state() == 0) &&
             (target3Resolution->get_state() == 0))
      newTargets = 0;
    else if (((target1Resolution->get_state() == 0) ^
          (target2Resolution->get_state() == 0) ^
             (target3Resolution->get_state() == 0)) == 1)
      newTargets = 1;
    else if (((target1Resolution->get_state() == 0 ) ^
          (target2Resolution->get_state() == 0) ^
             (target3Resolution->get_state() == 0)) == 2)
      newTargets = 2;
    else if ((target1Resolution->get_state() != 0) &&
          (target2Resolution->get_state() != 0) &&
             (target3Resolution->get_state() != 0))
